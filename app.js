...
let anime = [];
    ...
    try {
        const response = await axios.get('https://gogoanime.bid/');
        const $ = cheerio.load(response.data);
        const animeElements = $(' div.last_episodes.loaddub > ul > li > div > a > img');
        ...
        animeElements.each((index, element) => {
            const anime = {
                ...
                videoLink: "https://www1.gogoanime.bid"+$(element).parent().attr('href'),
            };
...
async function test(value,chap){
...
result.on('end',  async ()=>  {
  ...
  try{
    ...
    title = [...html.matchAll(/<h3 class="dib mb0(.*?)<\/h3>/gm)].map(match => match[1]);
...
let clean10=replaceAll(clean9,"Support our Vipnovel(com)","");
let clean11=replaceAll(clean10,'...',"");
let clean12=replaceAll(clean11,'\n\n',' ');
let clean13=replaceAll(clean12,'Support our Vipnovel(com)',"");
...
let clean25=clean24.replaceAll("© 2019 VipNovel. All rights reserved","");
...
// content = content.filter((month,idx) => idx < len-10)
  ...
  try{
       ...
       name = [...html.matchAll(/<h3 class="h4"><a href="https:\/\/vipnovel.com\/vipnovel\/(.*?)<\/a>/gm)].map(match => match[1]);
